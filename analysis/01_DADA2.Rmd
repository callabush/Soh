...
5. Infer errors on forward 

6. Identified ASVs on forward reads.
...
forward_reads <- 
  list.files(raw_fastqs_path, pattern = "_R1.fastq.gz", full.names = TRUE) 
#Intuition check
head(forward_reads)
```
...
# 515 F primer as described in the 16S Illumina Amplicon Protocol 
#from the Earth Microbiome Project
# 515 F GTG CCA GCM GCC GCG GTA A
...
raw_ASV_table <- makeSequenceTable(dada_forward)
...
raw_ASV_table_trimmed <- 
  raw_ASV_table[,nchar(colnames(raw_ASV_table)) %in% 134]
...
# Plot it!
track_counts_df %>%
  pivot_longer(input:nochim, 
               names_to = "read_type", values_to = "num_reads") %>%
...
1. Two ASV Count tables: 
      a. With ASV seqs: ASV headers include the *entire* ASV sequence ~134 bps.
...
saveRDS(noChimeras_ASV_table, 
     file = "data/Soh_analysis/noChimeras_ASV_table.RDS")
saveRDS(asv_tab, file = "data/Soh_analysis/ASV_counts.RDS")
...
saveRDS(track_counts_df, 
     file = "data/Soh_analysis/track_read_counts.RDS")
```
