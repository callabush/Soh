...
forward_reads <- list.files(raw_fastqs_path, pattern = ".fastq.gz",
                            full.names = TRUE) 
#Intuition check
head(forward_reads)
...
forward_filteredQual_plot <- plotQualityProfile(forward_reads) + 
...
samples <- scan("/local/workdir/cab565/git_repos/Soh/samples.txt", what = "character")
#Intuition check
head(samples)
...
92 percent of reads got through. Looks good. 
...
Points follow black lines. A2G and C2G look to have a few points off the black line. 
...
1. Two ASV Count tables: 
      a. With ASV seqs: ASV headers include the *entire* ASV sequence ~151 bps.
